...
// $pass = $_GET["student_pass"];
    ...
    $sql = "select * from gdfproject_student_insert g LEFT JOIN gdfproject_course_module c ON g.course_id=c.id where g.registeration_no='$user'";
    ...
    if(mysqli_num_rows($result)>0)
    {
        if($row=mysqli_fetch_assoc($result))
        {
            ...
            header("Location:veryfy.php");
        } 
      }
        else {
  echo "<script>
  alert('invalid ID');
  window.location='student_veryfy.php';
  </script>";
}
